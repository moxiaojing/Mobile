...
;(function(){
	var isMove;
	$(".app_open .app_home").on("touchstart",function(){
		isMove = false;
		$(this).css("color","#4CAE4C");
	})
	
	$(".app_open .app_home").on("touchmove",function(){
		isMove = false;
	})
	
	$(".app_open .app_home").on("touchend",function(event){
		if(isMove){
			return;
		}
		var ev = event.changedTouches;
//		console.log($(this));
		$(".app_open").css("transform","scale(0)");
		$(this).css("color","#fff");
